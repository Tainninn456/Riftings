...
public class Tennis_Player : PlayParent
{
    ...
    public static void bmove()
    {
        ...
        if (Mathf.Abs(bx - px) > 175 * (Main.powerUps[1] + 1) * (Main.powerUps[2] + 1) / div / (Main.debuf[1] + 1) / (Main.debuf[2] + 1) && by < -84 + Main.powerUps[1] * 70 + Main.powerUps[2] * 70 || Main.over)//ゲームオーバー
        {
            Main.HeartAmount--;
            if (Main.HeartAmount == -1)
            {
                Main.over = true;
                if (Main.memory.GameScores[1] < timeCount)
                {
                    Main.memory.GameScores[1] = (int)timeCount;
                }
        ...
        else if (by < -84 + Main.powerUps[1] * 70 + Main.powerUps[2] * 70 )//衝突時キック
        {
            //Debug.Log(Mathf.Abs(bx - px));
            //Debug.Log(58 * (Main.powerUps[1] + 1) * (Main.powerUps[2] + 1) / div / (Main.debuf[1] + 1) / (Main.debuf[2] + 1));
            float dic;
            if (Mathf.Abs(bx - px) < 40)//跳ねる大きさ
            {
                dic = 1.2f;
            }
            else
            ...
            if (once)
            {
                ...
                Main.brigi.Sleep();
                Main.brigi.AddForce(new Vector2(0, (dic + addY) * Main.phases[0] / Main.phases[1]), ForceMode2D.Impulse);
                once = false;
            }
            hx = (int)Main.pTra.transform.localPosition.x;
            Main.brigi.AddForce(new Vector2((bx - px) * 0.0005f, 0), ForceMode2D.Impulse);
            Torque();
        }
