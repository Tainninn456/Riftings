...
public class Boring_Player : PlayParent
{
    ...
    public static void bmove()
    {
        ...
        if (Mathf.Abs(bx - px) > 90 * (Main.powerUps[1] + 1) * (Main.powerUps[2] + 1) / div / (Main.debuf[1] + 1) / (Main.debuf[2] + 1) && by < -60 + Main.powerUps[1] * 70 + Main.powerUps[2] * 70 || Main.over)//ゲームオーバー
        {
            Main.HeartAmount--;
            if (Main.HeartAmount == -1)
            {
                Main.over = true;
                if (Main.memory.GameScores[3] < timeCount)
                {
                    Main.memory.GameScores[3] = (int)timeCount;
                }
        ...
        else if (by < -60 + Main.powerUps[1] * 70 + Main.powerUps[2] * 70)// - Main.debuf[1] * 70 - Main.debuf[2] * 70 - Main.phases[3] * 1.875f)//衝突時キック
        {
            float dic;
            if (Mathf.Abs(bx - px) < 10)//跳ねる大きさ
            {
                dic = 1.3f / Main.phases[1];
            }
            else
            {
                dic = 0.55f;
