using UnityEngine;
public class Vary_Player : PlayParent
{
    ...
    public static void bmove()
    {
        ...
        if (Mathf.Abs(bx - px) > 70 * (Main.powerUps[1] + 1) * (Main.powerUps[2] + 1) / div / (Main.debuf[1] + 1) / (Main.debuf[2] + 1) && by < -85 + Main.powerUps[1] * 70 + Main.powerUps[2] * 70 || Main.over)//ゲームオーバー
        {
            Main.HeartAmount--;
            if (Main.HeartAmount == -1)
            {
                Main.over = true;
                if (Main.memory.GameScores[8] < timeCount)
                {
                    Main.memory.GameScores[8] = (int)timeCount;
                }
        ...
        else if (by < -85 + Main.powerUps[1] * 70 + Main.powerUps[2] * 70)//衝突時キック
        {
            if (once)
            {
                Main.warpbool = false;
